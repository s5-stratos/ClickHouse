...
# set some vars
export CLICKHOUSE_PREFIX="${CLICKHOUSE_PREFIX:-/opt/clickhouse}"
export CLICKHOUSE_CONFIG="${CLICKHOUSE_CONFIG:-${CLICHOUSE_PREFIX}/clickhouse-server/config.xml}"
if [ x"$UID" == x0 ]; then
...
if [ -n "$(ls /docker-entrypoint-initdb.d/)" ]; then
    $gosu ${CLICKHOUSE_PREFIX}/bin/clickhouse-server --config-file=$CLICKHOUSE_CONFIG &
    pid="$!"
...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
    exec $gosu ${CLICKHOUSE_PREFIX}/bin/clickhouse-server --config-file=$CLICKHOUSE_CONFIG "$@"
fi
